...
//THESE ARE THE CARDGAME OBJECTS	
	...
	private ArrayList<String> gameDeck; //may not need to use this
	private int gamePhase;
	private int oldPhase;
	private int playersReached;
	//Date startDate; //Don't Construct until gameSetup is called?
	private long startTime;
	
	private Player whoseTurn;
	private int countA;
	private int currentPlyr;
	private boolean prevBet;
	...
	public Cardgame() {
		...
		//objMsgCh = null; //COULD BE A PROBLEM?????????????????????????????????????????????????
		startTime = 0;
		gameTable = new Table();
		// 0 no game; 1 gamePhase(); 2 40 seconds passed or 10 players firstround
		gamePhase = 0;
		playersReached = 0; //keeps a count of how many players have been able to bet
		whoseTurn = null;//gamePlayers.get(0);
	...
	public MessageChannel getmyChannel() {
		return objMsgCh;
	}
...
//	}
	...
	public void sendImage(MessageChannel channel) {
		...
		EmbedBuilder embed = new EmbedBuilder();
		File file = new File("cat_01.gif");
		embed.setImage("https://http.cat/500") // we specify this in sendFile as "cat.gif"
		      .setDescription("description here");
	...
	public void createDeck() {
		//construct deck
		gameDeck = new ArrayList<String>();
		
		gameDeck.add("2 of Spades (black)");gameDeck.add("2 of Clubs (black)");gameDeck.add("2 of Diamonds (red)");gameDeck.add("2 of Hearts (red)");
	    gameDeck.add("3 of Spades (black)");gameDeck.add("3 of Clubs (black)");gameDeck.add("3 of Diamonds (red)");gameDeck.add("3 of Hearts (red)");
	    gameDeck.add("4 of Spades (black)");gameDeck.add("4 of Clubs (black)");gameDeck.add("4 of Diamonds (red)");gameDeck.add("4 of Hearts (red)");
	    gameDeck.add("5 of Spades (black)");gameDeck.add("5 of Clubs (black)");gameDeck.add("5 of Diamonds (red)");gameDeck.add("5 of Hearts (red)");
	    gameDeck.add("6 of Spades (black)");gameDeck.add("6 of Clubs (black)");gameDeck.add("6 of Diamonds (red)");gameDeck.add("6 of Hearts (red)");
	    gameDeck.add("7 of Spades (black)");gameDeck.add("7 of Clubs (black)");gameDeck.add("7 of Diamonds (red)");gameDeck.add("7 of Hearts (red)");
	    gameDeck.add("8 of Spades (black)");gameDeck.add("8 of Clubs (black)");gameDeck.add("8 of Diamonds (red)");gameDeck.add("8 of Hearts (red)");
	    gameDeck.add("9 of Spades (black)");gameDeck.add("9 of Clubs (black)");gameDeck.add("9 of Diamonds (red)");gameDeck.add("9 of Hearts (red)");
	    gameDeck.add("10 of Spades (black)");gameDeck.add("10 of Clubs (black)");gameDeck.add("10 of Diamonds (red)");gameDeck.add("10 of Hearts (red)");
	    gameDeck.add("Jack of Spades (black)");gameDeck.add("Jack of Clubs (black)");gameDeck.add("Jack of Diamonds (red)");gameDeck.add("Jack of Hearts (red)");
	    gameDeck.add("Queen of Spades (black)");gameDeck.add("Queen of Clubs (black)");gameDeck.add("Queen of Diamonds (red)");gameDeck.add("Queen of Hearts (red)");
	    gameDeck.add("King of Spades (black)");gameDeck.add("King of Clubs (black)");gameDeck.add("King of Diamonds (red)");gameDeck.add("King of Hearts (red)");
	    gameDeck.add("Ace of Spades (black)");gameDeck.add("Ace of Clubs (black)");gameDeck.add("Ace of Diamonds (red)");gameDeck.add("Ace of Hearts (red)");
	...
	public void gameSetup(MessageReceivedEvent evt) {
		if(gamePhase != 0) {return;}//exit method 
		
		User objUser = evt.getAuthor();
		Message objMsg = evt.getMessage();

		if(objMsg.getContentRaw().equalsIgnoreCase(">gameSetup")) {
			setgamePhase(1);//set gamephase to 1
			createDeck(); //construct the deck
			...
			//objMsgCh.sendMessage(objUser.getAsMention()+"THIS MESSAGE NO LONGER EXISTS. *test*").queue();
		}
	}
	
	public void addPlayers(MessageReceivedEvent evt) {
		if(gamePhase != 1) {return;}
		
		User objUser = evt.getAuthor();
		Message objMsg = evt.getMessage();
		
		//Date currentDate = new Date(); //store the current date
		//int numSeconds = (int)((currentDate.getTime() - startDate.getTime()) / 1000);
		//long estimatedTime = System.currentTimeMillis() - startTime;
		
		long estimatedTime = (System.nanoTime() - startTime)/1000000000;
		if(objMsg.getContentRaw().equalsIgnoreCase(">joinGame") && estimatedTime > 40) {
			objMsgCh.sendMessage(objUser.getAsMention()+" Sorry, you cannot join the game later than 40 seconds after start. Current Time: "+estimatedTime+" seconds").queue();
			setgamePhase(2);//makes this method closed
		}
		if(objMsg.getContentRaw().equalsIgnoreCase(">joinGame") && gamePlayers.size() == 10) {
			objMsgCh.sendMessage(objUser.getAsMention()+" Sorry, you cannot join the game. There are already 10 players").queue();
			setgamePhase(2);//makes this method closed
		}
		//give the users 40 seconds to join the game after gameSetup
		if(objMsg.getContentRaw().equalsIgnoreCase(">joinGame") &&  estimatedTime < 40 && gamePlayers.size() <= 9) {
			Player p = new Player();
			p.setName(objUser.getName());
			p.setUser(objUser);
			
			deal(p);deal(p); //deal first two cards
			
			gamePlayers.add(p);
			objMsgCh.sendMessage(objUser.getAsMention()+" Has joined the game! Time: "+estimatedTime).queue();
			p.sendPrivateMessage("You have joined the Poker game. Your cards will be displayed in this private channel.");
			p.sendPrivateMessage(p.handtoString());
			//p.getChannel().sendFile("C://Users/Adam Blake Bell/Pictures/newpicc.jpg"); //use forward slashes!
			
			//sendImage(p.getChannel());
		}
	}
	
	public void getPlayers(MessageReceivedEvent evt) {
		if(gamePhase == 0) {return;}
		User objUser = evt.getAuthor();
		Message objMsg = evt.getMessage();
		
		if(objMsg.getContentRaw().equalsIgnoreCase(">getPlayers")) {
			objMsgCh.sendMessage(objUser.getAsMention()+" Asked for a list of Players in this game: ").queue();
			for(int i=0; i<gamePlayers.size();i++) {objMsgCh.sendMessage(gamePlayers.get(i).getName()).queue();}//print contents of gamePlayers
	...
	//NEED A CUSTOM EVENT??
//	public void getBet(MessageReceivedEvent evt) {
//		User objUser = evt.getAuthor();
//		Message objMsg = evt.getMessage();
//		if(gamePhase == 2) {
//		I
...
//	public void getBet(MessageReceivedEvent evt, int thisPhase) {
//		User objUser = evt.getAuthor();
//		Message objMsg = evt.getMessage();
//		
//		if(gamePhase == thisPhase){
//		objMsgCh.sendMessage(p.getName()+"It is your turn. Say '>Bet x' x=amountofChips or '>Fold' or '>FoldHide'").queue();
//		}
//	}
//
//	
//	public void firstRound(MessageReceivedEvent evt) {
//		if(gamePhase != 2) {return;}//only execute if gamephase == 2 //not needed here though
//		
//		if(gamePhase == 2) {
//		setgamePhase(3);//stops the method from hitting this part again
//		
//		objMsgCh.sendMessage("The first round of betting will commence.").queue();
//		
//		for(int i=0; i<gamePlayers.size();i++) {
//			getBet(evt, gamePlayers.get(i));
//		}
//		
//		
//		}
//	}
    ...
    public void monitorVariable() {
        new Thread(() -> {
            ...
            while (true) {
            	...
            	if(getmyChannel() != null && getStarttime() != 0 && gamePhase == 1) {
            	long estimatedTime = (System.nanoTime() - getStarttime())/1000000000;
            	if(estimatedTime > 40) {
            		getmyChannel().sendMessage("40 seconds has passed... No more players can join. Time: "+estimatedTime).queue();
            		getmyChannel().sendMessage("The first round of betting is about to begin.").queue();
            		gamePhase = 2;
            	}}
	...
	@SuppressWarnings("InfiniteLoopStatement")
    public void bettingThread() {
        new Thread(() -> {
            //log.info("Rotating Game Thread started");
            while (true) {
            	
            	int[] myarray = countNonfold();
            	
            	//if there is only one player who has not folded
            	if(myarray[0] == 1 && gamePhase == 2 && gamePlayers.size() != currentPlyr && prevBet == true) {
            		//reset several values
            		getmyChannel().sendMessage(gamePlayers.get(myarray[1]).getUser().getAsMention()+" Has won "+gameTable.getpot()+" chips!").queue();
            		gamePlayers.get(myarray[1]).addchips(gameTable.getpot());
            		gameTable.clearpot();
            		//set gamePhase
            		gamePhase = 0; //MAY NEED TO USE 1 here
            		resetplayersandtable(gamePlayers, gameTable);
              
            	}
            	
            	if(gamePhase == 2 && gamePlayers.size() != currentPlyr && prevBet == true && gamePlayers.get(currentPlyr).getfold() == 0) {
            	getmyChannel().sendMessage(gamePlayers.get(currentPlyr).getUser().getAsMention()+" It is your turn to bet. say '>Bet 10' where X is a number of chips").queue();
            	prevBet = false; //wait for them to bet before prompting again
            	betTime = System.nanoTime();//set a new timer
            	}
            	//skip over them if they folded
            	else if(gamePhase == 2 && gamePlayers.size() != currentPlyr && prevBet == true && gamePlayers.get(currentPlyr).getfold() > 0) {
        			//prevBet = true; //the player has bet!
        			currentPlyr++;//move to next player
        		}
	...
	public void getBet(MessageReceivedEvent evt) {
		...
		if(gamePlayers.size() == 0) {return;}
		if(currentPlyr >= gamePlayers.size()) {return;}
		
		User objUser = evt.getAuthor();
		Message objMsg = evt.getMessage();
		
		long estimatedTime = (System.nanoTime() - betTime)/1000000000; //problem here?
		
		if(objUser == gamePlayers.get(currentPlyr).getUser() && gamePlayers.get(currentPlyr).getfold() == 0) {
			if(objMsg.getContentRaw().equalsIgnoreCase(">Bet 10") && prevBet == false && estimatedTime < 30) {
				if(gamePlayers.get(currentPlyr).getchips() >= 10) {
					gamePlayers.get(currentPlyr).setrecentbet(10);//should change this method to automatically check if the bet is valid and add to totalbet	
					gamePlayers.get(currentPlyr).addtotalbet(10);
					gamePlayers.get(currentPlyr).removechips(10);
					gameTable.addpotchips(10);
					getmyChannel().sendMessage(gamePlayers.get(currentPlyr).getUser().getAsMention()+" has bet 10 chips! Time: "+estimatedTime).queue();
				}
				else {objMsgCh.sendMessage(gamePlayers.get(currentPlyr).getUser().getAsMention()+" You do not have enough chips.. Time: "+estimatedTime).queue();}
				prevBet = true; //the player has bet!
				currentPlyr++;//move to next player
			}
			else if(objMsg.getContentRaw().equalsIgnoreCase(">Fold") && prevBet == false && estimatedTime < 30) {
				gamePlayers.get(currentPlyr).setfold(1);
				getmyChannel().sendMessage(gamePlayers.get(currentPlyr).getUser().getAsMention()+" has FOLDED. "+gamePlayers.get(currentPlyr).handString()).queue();
				prevBet = true; //the player has bet!
				currentPlyr++;//move to next player
			}
			else if(objMsg.getContentRaw().equalsIgnoreCase(">FoldH") && prevBet == false && estimatedTime < 30) {
				gamePlayers.get(currentPlyr).setfold(1);
				getmyChannel().sendMessage(gamePlayers.get(currentPlyr).getUser().getAsMention()+" has FOLDED.").queue();
				prevBet = true; //the player has bet!
				currentPlyr++;//move to next player
			}
		}
	...
	//method that counts amount of people who havent folded
	public int[] countNonfold() {
		int counter = 0;
		int index = 0;;
		for(int i=0; i<gamePlayers.size(); i++) {
			if(gamePlayers.get(i).getfold() == 0) {
				counter++;
				index = i;
			}
		}
		int[] myarray = {counter, index};
		return myarray;
	}
	...
	public void shuffle(){
	  for(int i = 0; i<3; i++){
	  for (int counter = 0; counter < gameDeck.size(); counter++) {
	        ...
	        // objMsgCh.sendMessage(randomNumA+" "+randomNumB);
	         Collections.swap(gameDeck, i, randomNumA);
	     }}
	  //return gameDeck;
	}
	...
	public void deal(Player player){
	  player.addhand(gameDeck.get(gameDeck.size()-1));
	...
	/*
	                  ...
	                  //use kicker to determine winner
	                              ...
	                              //THIS WAS EDITED 12/40/2017 BECAUSE ONLY FIVE CARDS CAN BE USED
	                        //      else if(checkzA==1){
